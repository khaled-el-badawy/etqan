...
function CustomerOTP() {
  ...
  if (/^\d?$/.test(value)) { 
    ...
    // التنقل للفيلد اللي بعده تلقائياً إذا أدخل الرقم
    if (value && index < otp.length - 1) {
      const nextInput = document.getElementById(`otp-${index + 1}`);
      if (nextInput) nextInput.focus();
    }

    // إذا المسح Backspace، ارجع للفيلد اللي قبله
    if (!value && index > 0) {
  ...
  return (
    <div className="CustomerOTP-container">
      <motion.div 
      ...
      >
        ...
        <div className="CustomerOTP-form-fields">
          <form>
            ...
            >
              <button type="button" className="btn-CustomerOTP-container">تأكيد</button>
            </Link>
          </form>
      ...
      <div className="CustomerOTP-image">
        <motion.img 
