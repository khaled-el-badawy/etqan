...
<<<<<<< HEAD
      birthDate: birthDate, // نرسله كـ String "YYYY-MM-DD"
      maritalStatus: maritalList.indexOf(maritalStatus), // يرسل 0 أو 1 أو 2 أو 3
      nationalId: nationalId,
      phoneNumber: phone,
      jobId: jobsList.indexOf(selectedJob) + 1, // نفترض أن الـ IDs في الداتا بيز تبدأ من 1
    };

    try {
      const response = await axios.post(
        "https://etqanproject.runasp.net/api/ArtisanAccount/register-step1-send-otp",
        registerData,
      );

      if (response.status === 200) {
        // نمرر الإيميل لصفحة الـ OTP
=======
...
<<<<<<< HEAD
        ...
        //   "هناك خطأ في البيانات، تأكد أنك لم تسجل بهذا الإيميل من قبل.",
        // );
      } else {
...
<<<<<<< HEAD
              <div
                className="field-container marital-dropdown"
...
<<<<<<< HEAD
                  ...
                  <ul className="phone-errors">
                    ...
                    {phone.length >= 1 && phone[0] !== "0" && (
                      <li>رقم الهاتف يبدأ بـ 0</li>
                    )}
                    {phone.length >= 3 &&
                      !["010", "011", "012", "015"].includes(
                        phone.substring(0, 3),
                      ) && <li>بداية غير صحيحة</li>}
=======
                phone[0] !== "0" ||
                (phone.length >= 3 &&
                  !["010", "011", "012", "015"].includes(phone.substring(0, 3))) ? (
...
<<<<<<< HEAD
                ...
                {passwordsNotMatch && (
                  <p className="password-error-msg">كلمة السر غير مطابقة</p>
=======
                  type={showConfirmPassword?"text":"password"}
...
<<<<<<< HEAD
            ...
            <h4 className="h4-craftsman-login">
              هل لديك حساب ؟{" "}
              <Link to="/login/craftsman" className="Link">
                تسجيل الدخول
              </Link>
=======
