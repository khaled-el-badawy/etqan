...
import "./CompanyNewPassword.css";
import { FiEye, FiEyeOff } from "react-icons/fi";
import { motion } from "framer-motion";
import { Link, useNavigate } from "react-router-dom";

function CompanyNewPassword() {
  ...
  return (
    <div className="CompanyNewPassword-container">
      <motion.div
        className="CompanyNewPassword-form"
        initial={{ y: 60, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 1, ease: "easeOut", delay: 0.4 }}
      >

        <div className="CompanyNewPassword-form-fields">
          <form onSubmit={(e) => e.preventDefault()}>
            <h1 className="title">تعيين كلمة مرور جديدة</h1>

                      {/* كلمة المرور الجديدة */}
                      
            <div className="field-container CompanyNewPassword-password-field">
              <input
              ...
              {passwordError && (
                <p className="CompanyNewPassword-error-msg">{passwordError}</p>
              )}
            </div>
            ...
            <div className="field-container confirm-password-field">
              ...
              {(confirmPassword && confirmPassword !== password) && (
                <p className="CompanyNewPassword-error-msg">
                  يجب أن تكون كلمة السر مطابقة
                </p>
              )}

              {confirmPasswordError && (
                <p className="CompanyNewPassword-error-msg">{confirmPasswordError}</p>
              )}
            </div>
...
export default CompanyNewPassword;