...
```
📁 إتقان (Etqan)
├── 🏠 صفحة الهبوط — اختيار نوع المستخدم (عميل / حرفي / شركة) مع أنيميشن متقدم
├── 🔑 نظام المصادقة
│   ├── تسجيل الدخول (حسب الدور: /login/:role)
│   ├── تسجيل الدخول بـ OTP (عداد 60 ثانية + إعادة إرسال + API جاهز)
│   ├── نسيت كلمة المرور (تحقق بريد إلكتروني)
│   ├── التحقق من OTP (4 خانات + انتقال تلقائي بين الحقول)
│   ├── كلمة مرور جديدة (قواعد: حرف كبير + رمز خاص + 8 أحرف كحد أدنى)
│   ├── تسجيل عميل (اسم + بريد + هاتف + محافظة)
│   ├── تسجيل حرفي (+ تاريخ ميلاد + حالة اجتماعية + رقم قومي 14 رقم + المهنة)
│   ├── تسجيل شركة (+ رفع السجل التجاري PDF/JPG/PNG)
├── 🏡 الصفحة الرئيسية
│   ├── Hero Section — عدّاد إحصائيات متحرك (+500 حرفي، +1300 عميل، 4.8★)
│   ├── الخدمات الرئيسية
│   ├── أفضل الحرفيين (Swiper)
│   ├── آراء العملاء (Swiper)
│   └── تواصل معنا
├── 🔧 صفحة الخدمات — 20+ تخصص مع بحث وفلترة
├── 👷 صفحة الحرفيين — تصفح حسب التخصص مع بحث بالاسم والسعر + زر "عرض المزيد"
├── 🏢 دليل الشركات — تصنيفات (مقاولات، نقل مخلفات، رمل وزلط، تأجير قلابات ولودر، نقل أثاث)
├── 📋 مزودي الخدمات — قائمة شركات ديناميكية حسب التصنيف المختار
├── 🛒 المتجر — منتجات، ماركات، عروض، بحث، خريطة، تقييمات
├── 🏷️ صفحة الماركات — 72 منتج من 8 ماركات عالمية مع بحث ومفضلة وسلة
├── 📦 تفاصيل المنتج — معرض صور + تقييمات + تعليقات + منتجات مشابهة
├── ❤️ المفضلة — منتجات محفوظة مع إضافة جماعية للسلة + إحصائيات
├── 🛍️ سلة المشتريات — تحكم بالكميات (حد أقصى 5)، ملخص، إتمام الشراء
├── 🧾 الفاتورة — فاتورة مفصلة مع طرق الدفع
├── 👤 الملفات الشخصية
│   ├── ملف الحرفي — معرض أعمال، تقييمات، نموذج تعديل + طلب خدمة
│   ├── ملف الشركة — إحصائيات، خدمات (4 أقسام)، تقييمات + طلب خدمة مع اختيار محافظة
│   └── ملف العميل — سجل خدمات، تقييمات تفاعلية (نجوم)، شكاوى عبر Modal
├── 📋 إدارة الطلبات
│   ├── طلبات العميل — فلاتر (الكل / مكتمل / قيد التنفيذ / في الانتظار / ملغى) + تتبع 5 مراحل
│   ├── طلبات الحرفي — لوحة إدارة طلبات مخصصة
│   └── طلبات الشركة — لوحة إدارة مشاريع
├── 🔍 تفاصيل الطلب — حالة + بيانات + إلغاء مع إعادة توجيه تلقائي
├── 💬 صفحة المحادثة
├── ℹ️ من نحن
├── 📞 تواصل معنا
└── 🚫 صفحة 404
